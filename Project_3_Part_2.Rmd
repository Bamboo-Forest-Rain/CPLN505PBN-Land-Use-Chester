...
# pred_dat$Pred_Age43_Inc50K<- predict(mod_drive, newdat_gg, type="response")
# pred_dat$Pred_Age23_Inc50K<- predict(mod_drive, newdat_gg_1, type="response")
# pred_dat$Pred_Age60_Inc50K<- predict(mod_drive, newdat_gg_2, type="response")
# 
...
#calculate predicted probaviliites 
pred <- as.data.frame(fitted(mod_drive))
pred <- rename(pred, "prob" = "fitted(mod_drive)")
...
#anova
anova(mod_drive, test="Chisq") #anova tells us that the following are statistically significant contributions: Trip duration, gender, age, total vehicles, household size, no work 

mod_drive_2 <- glm ( drove_work ~  TRPDUR  + GEND + AGE + TOTVEH +  HHSIZE +   NOWRK, data=trips_person_hh, family = binomial)
summary(mod_drive)


#calculate predicted probaviliites 
pred <- as.data.frame(fitted(mod_drive_2))
pred <- rename(pred, "prob" = "fitted(mod_drive_2)")
pred <- mutate(pred, "binary" = ifelse(prob < 0.5, 0, 1))
#append to original df
trips_person_hh$binary <- pred$binary
head(trips_person_hh)
       
(sum(trips_person_hh$drove_work == 1 & trips_person_hh$binary == 1) + sum(trips_person_hh$drove_work == 0  #92%: removing the extra variables did almost nothing
  & trips_person_hh$binary == 0)) / nrow(trips_person_hh)


#collinearity 
vif(mod_drive_2) #all good - all under 2
...
```{r fwd and bwd selection}
#Backward Selection
step(glm(drove_work ~ TRPDUR  + GEND + AGE + TOTVEH +  HHSIZE +   NOWRK, family = binomial, data = trips_person_hh), direction = "backward")

#Forward Selection
fullmod<-glm(drove_work ~ TRPDUR  + GEND + AGE + TOTVEH +  HHSIZE +   NOWRK, family = binomial, data = trips_person_hh)
...
mod_drive_3 <- glm(drove_work ~ TOTVEH + NOWRK + TRPDUR + AGE + GEND, family = binomial, data = trips_person_hh)
summary(mod_drive_3)
...
exp(coef(mod_drive_3))
100 * (exp(coef(mod_drive_3))-1)
```

## Model 2: Walk/bike to work or not   
```{r walk bike}
...
mod_bike_2 <- glm ( bike_walk_work ~  TRPDUR  +  AGE + HHSIZE +  INCOME +  NOWRK, data=trips_person_hh, family = binomial) #Trip duration, age, total vehicle, household size, income, no work
summary(mod_bike_2)



#calculate predicted probaviliites 
pred <- as.data.frame(fitted(mod_bike_2))
pred <- rename(pred, "prob" = "fitted(mod_bike_2)")
pred <- mutate(pred, "binary" = ifelse(prob < 0.5, 0, 1))
#append to original df
trips_person_hh$binary <- pred$binary
head(trips_person_hh)
       
(sum(trips_person_hh$drove_work == 1 & trips_person_hh$binary == 1) + sum(trips_person_hh$drove_work == 0  #10%: a little better
  & trips_person_hh$binary == 0)) / nrow(trips_person_hh)
...
#Backward Selection
step(glm(bike_walk_work ~  TRPDUR  +  AGE + HHSIZE +  INCOME +  NOWRK, family = binomial, data = trips_person_hh), direction = "backward")

#Forward Selection
fullmod<-glm(bike_walk_work ~TRPDUR  +  AGE + HHSIZE +  INCOME +  NOWRK, family = binomial, data = trips_person_hh)

intonly<-glm(bike_walk_work ~ 1, family = binomial, data = trips_person_hh)

step(intonly, scope=list(lower=intonly, upper=fullmod), direction="forward") #final : trip duration, age, household size


mod_bike_3 <- glm(bike_walk_work ~TRPDUR  +  AGE + HHSIZE, family = binomial, data = trips_person_hh)
summary(mod_bike_3)


#test fit
qchisq(.95, df=2915)       #doesn't pass


exp(coef(mod_bike_3))
100 * (exp(coef(mod_bike_3))-1)
```
