...
=======
trips <- read.csv("/Users/annaduan/Documents/planning\ by\ numbers/3/CPLN505PBN-Land-Use-Chester/HH\ Travel\ Survey/trip_pub.csv") %>%
>>>>>>> b62ad5b292b225262c2a0b88174adf5d70977c84
...
households <- read.csv("/Users/annaduan/Documents/planning\ by\ numbers/3/CPLN505PBN-Land-Use-Chester/HH\ Travel\ Survey/hh_pub_CSV.csv", header = TRUE) %>%
  dplyr::select(ADVLT, TOTVEH, DWELL, YRMOV, RENT, DIARY, ENGL, HHSIZE, NPHON, NOPHO, SHPHN, ETHNC, INCLV, INCOM, INCOME, ASSN, DPHON, NPLAC, NTRIPS, NOWRK, NOSTU, OWNSH, EACCT, CPA, DAYOFWK, HBW, HBO, NHB, MOTTRIP, SAMPN, LISTD, HADDR)

persons <- read.csv("/Users/annaduan/Documents/planning\ by\ numbers/3/CPLN505PBN-Land-Use-Chester/HH\ Travel\ Survey/per_pub.csv") %>%
>>>>>>> b62ad5b292b225262c2a0b88174adf5d70977c84
...
trips_person_hh_multi$FARE1[is.na(trips_person_hh_multi$FARE1)] <- 0
trips_person_hh_multi$FARE2[is.na(trips_person_hh_multi$FARE2)] <- 0
trips_person_hh_multi$FARE3[is.na(trips_person_hh_multi$FARE3)] <- 0
trips_person_hh_multi$PARKC[is.na(trips_person_hh_multi$PARKC)] <- 0
trips_person_hh_multi$TOLLA[is.na(trips_person_hh_multi$TOLLA)] <- 0
trips_person_hh_multi$TRFR[is.na(trips_person_hh_multi$TRFR)] <- 0

#Column with total Fare
trips_person_hh_multi <-
  trips_person_hh_multi %>%
  mutate(TotCost = FARE1 + FARE2 + FARE3 + TOLLA + PARKC)

trips_multi_data <- trips_person_hh_multi %>%
  select("TRPDUR",'GEND', 'DISAB', 'EDUC','TOTVEH','ADVLT', 'RENT', 'ENGL', 'HHSIZE', 'NPHON', 'INCOME', 'NOWRK','AGE', 'TotCost', 'TRFR', mode2)
...
ml.trips <- mlogit ( mode2 ~ TRPDUR + TotCost + TRFR | GEND + AGE+DISAB+EDUC + TOTVEH + ADVLT + RENT  + HHSIZE + NPHON + INCOME + NOWRK, TM)
